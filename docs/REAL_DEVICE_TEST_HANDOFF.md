...
実装済みの自動検証では、ゲームロジック17件とブラウザE2E 3件が成功しています。実機でしか確認できない項目は、カメラ推論の操作感、Position ID、車輪方向、LED、内蔵音、BLE切断時の停止です。
...
5. 最低1回の壁発見と1回の得点を目標にします。
6. 0秒でtoioが停止し、`EXPLORATION COMPLETE`になることを確認します。
